  ...
  for (int y = 0; y < sideSizeCached; y++)
    for (int x = 0; x < sideSizeCached; x++) {
      ...
      if (h < 0.3)
        ...
        colors.push_back(Color32::LerpColor(Color::Yellow, Color::Green, (h - 0.4f) / 0.1f));
      else if (h < 0.7)
        colors.push_back(Color32::LerpColor(Color::Green, Color::SandyBrown, (h - 0.5f) / 0.2f));
      else
        colors.push_back(Color32::LerpColor(Color::SandyBrown, Color::White, (h - 0.7f) / 0.3f));
    }
  return colors;
}
...
glm::vec3 ParticleGenerator::surfaceNormal(int i, int j) {
  ...
  */
  ...
  n += glm::vec3(0.1)
       * glm::normalize(glm::vec3(scale * (heights[i][j] - heights[i + 1][j + 1]) / sqrt2, sqrt2,
                                  scale * (heights[i][j] - heights[i + 1][j + 1]) / sqrt2));  // Positive Y
  n += glm::vec3(0.1)
       * glm::normalize(glm::vec3(scale * (heights[i][j] - heights[i + 1][j - 1]) / sqrt2, sqrt2,
                                  scale * (heights[i][j] - heights[i + 1][j - 1]) / sqrt2));  // Positive Y
  n += glm::vec3(0.1)
...
void ParticleGenerator::Erode(float dt) {
  ...
  for (int i = 0; i < cycles; i++) {
    ...
    while (drop.volume > minVol) {
      ...
      if (ipos.x <= 0 || ipos.y <= 0 || ipos.x >= sideSizeCached - 1 || ipos.y >= sideSizeCached - 1) break;
    ...
    std::cout << "[" << startPos.x << "," << startPos.y << "]"
              << "->[" << (int)drop.pos.x << "," << (int)drop.pos.y << "] angle: " << std::fixed << std::setprecision(2)
              << atan2(delta.y, delta.x) * (180 / 3.1415) << " dist: " << glm::length(delta) << std::endl;
  }
